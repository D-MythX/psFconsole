...
echo -e "\e[1;92m Setting up environment for \e[0m\e[1;94mosmconsole...\e[0m"
sleep 0.5
clear
trap 'printf "\n";stop;exit 1' 2
...
export RAWLINK=https://raw.githubusercontent.com/anonynusman/pentesting-framework/master
export GITLINK=https://github.com/anonynusman/pentesting-framework
...
source $CWD/assets/programs/info.osmconsole
else
source <(curl -sLo- "${RAWLINK}/assets/programs/list.modules")
source <(curl -sLo- "${RAWLINK}/assets/programs/info.osmconsole")
fi
...
if [[ ! $__return__ == 105 ]];then
source $CWD/assets/programs/requirements.osm
else
source <(curl -sLo- "${RAWLINK}/assets/programs/requirements.osm")
fi
...
interruptPSF() {
printf "                                                  \r"
echo -e "${r0SIGN} Osmconsole is Interrupted                "
exit 0
}
...
================
    ...
    ${C2}quit${R0}        Command to exit osmconsole instance.
...
\`${C4}osmconsole${R0}\` is a bundle of penetration testing tools and an interface to Pentesting Framework.
...
Type: "show modules" (without quotes) to display available modules
...
if [[ $? == 0 ]]
then
echo "Online             Osm have proper connection"
else
echo "Offline            Osm doesn't have connection"
fi
}
...
type() {
if [[ ! $__return__ == 105 ]];then
echo "Offline            Osm is running on system"
else
echo "Online             Osm is running on temp"
fi
}
TYPE=$(printf "$(type)")
}
...
if [[ $VAR != $UPDATE ]]
then
STATUS=$(echo "Available          Update command: osm update")
else
...
===============
   ...
   Prompt             osm prompt=$PROMPT         The prompt string
...
if [[ ! $__return__ == 105 ]];then
source $CWD/assets/programs/setup.osmconsole
else
source <(curl -sLo- "${RAWLINK}/assets/programs/setup.osmconsole")
fi
exit 0

elif [[ ${1} == "-u" || ${1} == "--update" ]];then
if [[ ! $__return__ == 105 ]];then
source $CWD/assets/programs/update.osmconsole
else
source <(curl -sLo- "${RAWLINK}/assets/programs/update.osmconsole")
fi
exit 0

elif [[ ${1} == "-r" || ${1} == "--remove" ]];then
if [[ ! $__return__ == 105 ]];then
source $CWD/assets/programs/remove.osmconsole
else
source <(curl -sLo- "${RAWLINK}/assets/programs/remove.osmconsole")
fi
exit 0
fi
...
then
echo -e "
Usage: osmconsole [options]

Module options:
       show modules=[to display available modules]
       list=[to display a menu of a selected module]
...
case ${RANDOMs} in
1)
       printf "Osmconsole tip: When in a module, use ${C2}back${R0} to go\n";WAIT
       printf "back to the top level prompt\n\n";WAIT
       ;;
2)
       printf "Osmconsole tip: You can use ${C2}help${R0} to view all\n";WAIT
...
theHEADER() {
randomBANNER
printf "+ -- --=[ ${C3}osmconsole release v${VAR}${R0} ]=-- -- +\n";WAIT
...
overHISTORY() {
TOT=$((LOG-30))
sed -n "${TOT},${LOG}p" $OUTPUT > .temp.history.osm && mv .temp.history.osm $OUTPUT
}
...
getCOMMANDS() {
  if [[ ${command} == "use with:phish/otpweb/handler" ]]
   ...
   then
     ...
     then
        echo -e "${g0SIGN} Existing osmconsole ..."
        exit 0
        command=""

  elif [[ ${command} == "osm update" ]]
     then
       git pull
       command=""
       exit 0
...
To display available modules type "show modules" (without quotation marks)
...
list: to display available items/tools
"
        command=""
...
readCONFIGURE () {
  ...
  then
     ...
     if [[ ! ${FALSE} == "false" ]]
     then
     read -r -p $'\e[4mosm1\e[0m use(\e[91mwebpage/phish/otpweb\e[0m) > ' input
     fi
     BUNDLE()
{
  ...
  then
     ...
     if [[ ! ${FALSE} == "false" ]]
     then
     read -r -p $'\e[4mosm\e[0m use(\e[91mwebpage/phish/notpweb\e[0m) > ' input
     fi
     BUNDLE()
{
  ...
  then
     ...
     if [[ ! ${FALSE} == "false" ]]
     then
     read -r -p $'\e[4mosm\e[0m use(\e[91mwebpage/phish/gpsweb\e[0m) > ' input
     fi
     BUNDLE()
{
  ...
  then
     ...
     if [[ ! ${FALSE} == "false" ]]
     then
     read -r -p $'\e[4mosm\e[0m use(\e[91mwebpage/phish/camweb\e[0m) > ' input
     fi
     BUNDLE()
{
  ...
  then
     ...
     if [[ ! ${FALSE} == "false" ]]
     then
     read -r -p $'\e[4mosm\e[0m use(\e[91mwebpage/phish/micweb\e[0m) > ' input
     fi
     BUNDLE()
{
  ...
  then
     ...
     if [[ ! ${FALSE} == "false" ]]
     then
     read -r -p $'\e[4mosm\e[0m use(\e[91mprogram/bomber/callnsms\e[0m) > ' input
     fi
     BUNDLE()
{
     if [[ $input == "list" ]]
...
case $(dpkg --print-architecture) in
  ...
  then
     ...
     if [[ ! ${FALSE} == "false" ]]
     then
     read -r -p $'\e[4mosm\e[0m use(\e[91mprogram/lookup/user\e[0m) > ' input
     fi
     BUNDLE()
{
readINPUT "list1-m3" "3" "withLOOKUP/USER/user"
  ...
  then
     ...
     if [[ ! ${FALSE} == "false" ]]
     then
     read -r -p $'\e[4mosm\e[0m use(\e[91mprogram/lookup/number\e[0m) > ' input
     fi
     BUNDLE()
{
readINPUTS "run" "source" "withLOOKUP/NUMBER/number"
  ...
  then
     ...
     if [[ ! ${FALSE} == "false" ]]
     then
     read -r -p $'\e[4mosm\e[0m use(\e[91mprogram/remote/payload\e[0m) > ' input
     fi
     BUNDLE()
{
  ...
  then
     ...
     if [[ ! ${FALSE} == "false" ]]
     then
     read -r -p $'\e[4mosm\e[0m use(\e[91mprogram/brute/social\e[0m) > ' input
     fi
     BUNDLE()
{
  ...
  then
     ...
     if [[ ! ${FALSE} == "false" ]]
     then
     read -r -p $'\e[4mosm\e[0m use(\e[91mprogram/brute/hashes\e[0m) > ' input
     fi
     BUNDLE()
{
     ...
     BUNDLE()
{
  ...
  then
     ...
     if [[ ! ${FALSE} == "false" ]]
     then
     read -r -p $'\e[4mosm\e[0m use(\e[91mprogram/secure/hashes\e[0m) > ' input
     fi
     command=""
     if [[ ! ${FALSE} == "false" ]]
     then
  ...
  then
     ...
     if [[ ! ${FALSE} == "false" ]]
     then
     read -r -p $'\e[4mosm\e[0m use(\e[91mprogram/secure/passwd\e[0m) > ' input
     fi
     BUNDLE()
{
